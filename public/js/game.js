...
var sketchAnswers = ["BOAT","VENON","STARWARS","TITANIC","APE","JACKET","SHARK","ELVIS","WOLF","DOG","SEASICK","ICECREAM","BALL","DANCE","CAT","MONKEY","ROCKET","UNICORN","SPACE","CARROT"];
...
function loadSketchAnswer(){
	...
	drawContainer.cache(-(gameData.boardMaskW/2),-(gameData.boardMaskH/2),gameData.boardMaskW,gameData.boardMaskH);
	...
	switch(con){
		case 'down':
			...
			if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online && socketData.turn) {
				postSocketUpdate('startdrawingline', {x:startPos.x, y:startPos.y}, true);
			}
		break;
		...
		case 'move':
			if (gameData.drawing) {
				...
				if ( typeof initSocket == 'function' && multiplayerSettings.enable && socketData.online && socketData.turn) {
					postSocketUpdate('updatedrawingline', {x:movePos.x, y:movePos.y}, true);
				}
			}
		break;
...
function updateDrawingLine(x, y){
	...
	drawContainer.updateCache("source-over");
	boardDrawing.graphics.clear();
}
